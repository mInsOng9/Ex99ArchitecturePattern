package com.song.mvvm.view

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import com.song.mvvm.R
import androidx.databinding.DataBindingUtil
import com.song.mvvm.databinding.ActivityMainBinding
import com.song.mvvm.viewmodel.ItemViewModel
...
class MainActivity : AppCompatActivity() {
    ...
    //MVVM의 장점
    //1. MVP의 view와 presenter처럼 1:1로 대응되어 잇지 않기에 뷰가 많아져도 뷰모델이 많아지지 않아도 됨.
    //2. 사용자 이벤트 처리를 모두 viewModel 역할 클래스에서 하고 있기에 중복되는 코드를 다시 작성하지 않고 재사용 가능함.
    //3. 데이터 바인딩이 되어 있기에 데이터가 변경되었을때 별도의 setText() or notifyDataSetChanged()같은 갱신코드가 필요 없이 자동 갱신됨.
    //4. Activity 코드가 가장 간결함.

    //MVVM의 단점
    //1. data binding 레이아웃 설계 작업이 다소 복잡함
    //2. view의 처리가 복잡할 수록 결국 viewModel 클래스가 엄청 비대해짐
