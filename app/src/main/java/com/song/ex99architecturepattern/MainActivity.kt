package com.song.ex99architecturepattern

import android.app.Activity
import android.content.SharedPreferences
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import com.song.ex99architecturepattern.databinding.ActivityMainBinding

class MainActivity : AppCompatActivity() {

    //Architecture Pattern 없이 그냥 작성하는 Flat Design
    //1) 장점 : 구조가 단순하여 구현하기 쉽고 하나의 문서에 대부분의 기능코드와 UI 코드가 있어서 전체기능이 한눈에 들어옴
    ...
    //# Data를 제어(CRUD 저장/읽기/삭제/변경)하는 작업을 하는 Business Logic 기능 메소드 [ex. 네트워크 통신,DB작업 등]
    private fun clickSave(){
        val pref:SharedPreferences = getSharedPreferences("data", MODE_PRIVATE)
        pref.edit().apply {
            this.putString("name",binding.etName.text.toString()) //this 생략 가능
